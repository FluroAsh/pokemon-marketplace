module PagesHelper
  def get_random_card_sprite # used on the home page
    ...
    if File.file?("#{Rails.root}/app/assets/images/#{card.sprite_name.downcase}.gif") # checks to see if file with sprite name exists (gif sprite)
      image_tag "#{card.sprite_name.downcase}.gif", class: "gif-sprite", title: card.name, alt: card.name
    else # backup sprite
      image_tag "#{url_lg_sprite(card.national_pokedex_number)}", class: "png-sprite", alt: card.name, title: card.name, style: "height:100px;"
    end
  end
end
